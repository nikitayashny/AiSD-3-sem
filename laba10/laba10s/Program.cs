class Program
{
    ...
    static double ETA = 100;    // величина, котрую нужно делить на расстояние между городами в формуле желания
    ...
    public struct Way
    {
        ...
        public void Add(int index)
        {
            if (!list.Contains(index))
            {
                if (list.Count != 0)
                {
                    length += matrix[list.Count - 1, index, 0];
                }
                list.Add(index);
            }
            else
            {
                Console.WriteLine("Этот город уже включён в путь");
            }
        }
    ...
    static void Main()
    {
        ...
        bool Contains(int x, int y)
        {
            ...
            {
                if (x == citiesCoordinates[i, 0] && y == citiesCoordinates[i, 1])
                {
                    return true;
                }
            }
        ...
        int NextCity(List<int> visited)
        {
            ...
            for (int m = 0; m < numOfCities; m++)
            {
                ...
                {
                    denominator += Math.Pow(matrix[currentIndex, m, 1], alpha) *
        ...
        for (int i = 0; i < iterCount; i++) // начало итераций
        {
            ...
            Way FindMin()
            {
                ...
                {
                    if (listOfWays[i].length < ret.length)
                    {
                        ret = listOfWays[i];
                    }
                }
            ...
            bestWay = FindMin();
            Console.WriteLine($"Лучший путь на итерации {i}: {bestWay}");
        }
    }
}